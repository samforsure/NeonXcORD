...
public class ThemingTools {
    ...
    public static void setWelcomeText(final TextView tv, final CharSequence text) {
        if (text.toString().equalsIgnoreCase("Welcome To NeonX Cord")) {
            ValueAnimator va = new ValueAnimator();
            va.setIntValues(
                    Color.parseColor("#FF0000"),   // NeonX Red
                    Color.parseColor("#CC0000"),   // Deep Red
                    Color.parseColor("#FF4444"),   // Neon Red
                    Color.parseColor("#FF6B6B"),   // Soft Neon
                    Color.parseColor("#FF0000")    // Back to NeonX Red
            );
            va.setEvaluator(new ArgbEvaluator());
            va.addUpdateListener(animation -> tv.setTextColor((Integer) animation.getAnimatedValue()));
            va.setRepeatCount(-1);
            va.setDuration(4000);  // faster, more energetic
            va.start();
        }
        tv.setText(text);
    }
    ...
    public static void setCreditsText(TextView tv) {
        ...
        sb.append(String.format("\nBased on Discord %s (%s)\n~Made with ❤️ by Rasengan~", pi.versionName, pi.versionCode));
